const STORAGE_KEYS = {
  ...
  themeMigration: "taskham.theme.migrated"
}
...
const APP_VERSION = window.TASKHAM_CONFIG?.version || "0.0.0"
const LANGUAGES = {
  fa: {
...
Object.assign(LANGUAGES.fa, {
  ...
  themeCount: "۶ حالت",
  unsafeText: "این متن شامل کد یا محتوای ناامن است. لطفا آن را به شکل متن ساده وارد کن.",
  openTaskComposer: "افزودن تسک جدید",
  appearanceGroup: "نمایش", appearanceHint: "حالت مناسب محیطت را انتخاب کن.",
  behaviorGroup: "رفتار برنامه", taskLimitHint: "سقف تسک‌های فعال را متناسب با روزت تنظیم کن.",
  readingGroup: "خوانایی", fontHint: "اندازه و وزن نوشتار را برای خواندن راحت‌تر تنظیم کن.",
  themeGroup: "رنگ و فضا", themeHint: "یک پالت هماهنگ برای ظاهر برنامه انتخاب کن.",
  privacyGroup: "حریم خصوصی", lockHint: "برای حریم خصوصی روزمره، یک پین چهاررقمی محلی بگذار."
...
Object.assign(LANGUAGES.en, {
  ...
  themeCount: "6 styles",
  unsafeText: "This text includes code or unsafe content. Please use plain text instead.",
  openTaskComposer: "Add a new task",
  appearanceGroup: "Appearance", appearanceHint: "Choose the mode that fits your environment.",
  behaviorGroup: "Behaviour", taskLimitHint: "Set an active-task limit that fits your day.",
  readingGroup: "Reading", fontHint: "Adjust text size and weight for comfortable reading.",
  themeGroup: "Colour and mood", themeHint: "Choose a cohesive palette for the app.",
  privacyGroup: "Privacy", lockHint: "Set a local four-digit PIN for everyday privacy."
...
function bindEvents() {
  ...
  ;[elements.taskDueInput, elements.detailDueInput].forEach((input) => input.addEventListener("focus", () => openJalaliPicker(input)))
  ...
  window.addEventListener("resize", () => { if (!isMobileViewport()) setMobileMenu(false); syncMobileTaskComposer() })
  ...
  elements.mobileAddBtn.addEventListener("click", openMobileTaskComposer)
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Escape" && !state.isLocked) { setMobileMenu(false); [elements.detailModal, elements.settingsModal, elements.historyModal, elements.trashModal, elements.confirmModal, elements.mobileTaskModal].forEach(closeModal) }
  })
...
function handleAddTask(event) {
  ...
  const title = sanitizePlainText(elements.taskInput.value, 50); const note = sanitizePlainText(elements.taskNoteInput.value, 500, true)
  const isMobileComposer = !elements.mobileTaskModal.hidden
  if (!title.value) return setStatus(t().statusEmpty, "error")
  if (title.unsafe || note.unsafe) return setStatus(t().unsafeText, "error")
  ...
  state.tasks.unshift({ id: crypto.randomUUID(), title: title.value, note: note.value, category: validCategory(elements.taskCategoryInput.value), priority: validPriority(elements.taskPriorityInput.value), status: validTaskStatus(elements.taskStatusInput.value), dueAt: dateInputToTimestamp(elements.taskDueValue.value), pinned: false, createdAt: Date.now(), updatedAt: null })
  saveTasks(); resetTaskComposer(); render(); if (isMobileComposer) closeModal(elements.mobileTaskModal); else elements.taskInput.focus(); setStatus(t().taskAdded, "success")
}

function handleSaveTask(event) {
  event.preventDefault(); const task = getSelectedTask(); const title = sanitizePlainText(elements.detailInput.value, 50); const note = sanitizePlainText(elements.detailNoteInput.value, 500, true); if (!task || !title.value) return
  if (title.unsafe || note.unsafe) return setStatus(t().unsafeText, "error")
  const category = validCategory(elements.detailCategoryInput.value)
  if (!category) return setStatus(t().categoryRequired, "error")
  task.title = title.value; task.note = note.value; task.category = category; task.priority = validPriority(elements.detailPriorityInput.value); task.status = validTaskStatus(elements.detailStatusInput.value); task.dueAt = dateInputToTimestamp(elements.detailDueValue.value); task.updatedAt = Date.now(); saveTasks(); render(); closeModal(elements.detailModal); setStatus(t().taskSaved, "success")
}
...
function handleSearch(event) { state.search = sanitizePlainText(event.target.value, 120, true).value; elements.clearSearchBtn.hidden = !state.search; window.clearTimeout(state.searchTimer); state.searchTimer = window.setTimeout(renderTasks, 120) }
function clearSearch() { window.clearTimeout(state.searchTimer); state.search = ""; elements.taskSearchInput.value = ""; elements.clearSearchBtn.hidden = true; renderTasks(); elements.taskSearchInput.focus() }
...
function renderThemeGrid() { elements.themeGrid.innerHTML = ""; THEMES.forEach((theme) => { const button = document.createElement("button"); const head = document.createElement("div"); const name = document.createElement("strong"); const icon = document.createElement("i"); const swatches = document.createElement("div"); button.type = "button"; button.className = "theme-card"; button.dataset.theme = theme.id; name.className = "theme-name"; name.textContent = themeLabel(theme.id); icon.className = "fa-solid fa-palette"; icon.setAttribute("aria-hidden", "true"); head.className = "theme-card-head"; head.append(name, icon); swatches.className = "theme-swatches"; theme.colors.forEach((color) => { const swatch = document.createElement("span"); swatch.style.background = color; swatches.appendChild(swatch) }); button.append(head, swatches); button.addEventListener("click", () => { state.theme = theme.id; localStorage.setItem(STORAGE_KEYS.theme, state.theme); applyTheme(); renderThemeGrid() }); button.classList.toggle("active", state.theme === theme.id); elements.themeGrid.appendChild(button) }) }
...
function applyTheme() { state.theme = resolveTheme(state.theme); document.documentElement.dataset.theme = state.theme }
...
function revealModal(element, focusTarget) { if (!element) return; const pendingClose = state.modalCloseTimers.get(element); if (pendingClose) { window.clearTimeout(pendingClose); state.modalCloseTimers.delete(element) } state.lastFocusedElement = document.activeElement instanceof HTMLElement ? document.activeElement : null; state.activeModal = element; element.hidden = false; element.setAttribute("aria-hidden", "false"); element.classList.remove("is-closing"); elements.appMain.inert = true; requestAnimationFrame(() => element.classList.add("is-open")); document.body.style.overflow = "hidden"; window.setTimeout(() => (focusTarget || getFocusableElements(element)[0])?.focus(), 60) }
function openModal(element, focusTarget) { if (!element || state.isLocked) return; revealModal(element, focusTarget) }
function closeModal(element) { if (!element || element.hidden || element.classList.contains("is-closing")) return; const returnFocus = state.lastFocusedElement; element.classList.remove("is-open"); element.classList.add("is-closing"); if (element === elements.detailModal) state.selectedTaskId = null; if (element === elements.confirmModal) state.pendingDeleteIds = []; const timer = window.setTimeout(() => { state.modalCloseTimers.delete(element); element.hidden = true; element.setAttribute("aria-hidden", "true"); element.classList.remove("is-closing"); if (element === elements.mobileTaskModal) returnTaskComposerHome(); const allModals = [elements.detailModal, elements.settingsModal, elements.historyModal, elements.trashModal, elements.confirmModal, elements.mobileTaskModal, elements.lockModal]; if (allModals.every((modal) => modal.hidden) && !state.isLocked) { document.body.style.overflow = ""; elements.appMain.inert = false; state.activeModal = null; (element === elements.mobileTaskModal ? elements.mobileAddBtn : returnFocus)?.focus?.() } }, MODAL_TRANSITION_MS); state.modalCloseTimers.set(element, timer) }
function getFocusableElements(container) { return [...container.querySelectorAll('button:not([disabled]), [href], input:not([disabled]):not([type="hidden"]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])')].filter((node) => !node.hidden && node.offsetParent !== null) }
function trapModalFocus(event) { const modal = state.activeModal; if (!modal || modal.hidden) return; const focusable = getFocusableElements(modal); if (!focusable.length) return; const first = focusable[0]; const last = focusable.at(-1); if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last.focus() } else if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first.focus() } }
function isMobileViewport() { return window.matchMedia("(max-width: 620px)").matches }
function openMobileTaskComposer() { if (!isMobileViewport()) return elements.taskInput.focus(); elements.mobileTaskMount.appendChild(elements.taskInputCard); openModal(elements.mobileTaskModal, elements.taskInput) }
function returnTaskComposerHome() { if (!elements.taskInputHome.contains(elements.taskInputCard)) elements.taskInputHome.appendChild(elements.taskInputCard) }
function syncMobileTaskComposer() { if (!isMobileViewport() && !elements.mobileTaskModal.hidden) closeModal(elements.mobileTaskModal); if (!isMobileViewport()) returnTaskComposerHome() }
function resetTaskComposer() { elements.taskForm.reset(); elements.taskNoteInput.value = ""; elements.taskCategoryInput.value = ""; elements.taskPriorityInput.value = ""; elements.taskStatusInput.value = "pending"; elements.taskDueInput.value = ""; elements.taskDueNative.value = ""; elements.taskDueValue.value = ""; updateCharCounter() }
...
function normalizeTask(item) { const title = sanitizePlainText(item?.title, 50); const note = sanitizePlainText(item?.note, 500, true); return { id: typeof item?.id === "string" && item.id ? item.id : crypto.randomUUID(), title: title.unsafe ? "" : title.value, note: note.unsafe ? "" : note.value, category: validCategory(item?.category), priority: validPriority(item?.priority), status: validTaskStatus(item?.status), dueAt: validTimestamp(item?.dueAt), pinned: Boolean(item?.pinned), createdAt: validTimestamp(item?.createdAt) || Date.now(), updatedAt: validTimestamp(item?.updatedAt) } }
...
function resolveTheme(value) { return THEMES.some((theme) => theme.id === value) ? value : THEMES[0].id }
...
function startJalaliDatepicker() { if (!state.jalaliStarted && window.jalaliDatepicker) { window.jalaliDatepicker.startWatch({ time: true, hasSecond: false, targetValueInput: "attr", targetValueType: "attr" }); state.jalaliStarted = true } }
...
function themeLabel(themeId) { const labels = { fa: { ocean: "ساحل", sunset: "مرجانی", forest: "خزه", noir: "جوهر", lavender: "مهتاب", ice: "یخچال" }, en: { ocean: "Coast", sunset: "Coral", forest: "Moss", noir: "Ink", lavender: "Moonlight", ice: "Glacier" } }; return labels[state.language]?.[themeId] || themeId }
...
function sanitizePlainText(input, maxLength, multiline = false) { if (typeof input !== "string") return { value: "", unsafe: false }; const normalized = input.normalize("NFC").replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F\u202A-\u202E\u2066-\u2069]/g, ""); const unsafe = /<\s*\/?\s*(?:script|iframe|object|embed|svg|img|style|link|base|meta)\b|\bon[a-z]+\s*=|\b(?:javascript|vbscript)\s*:|data\s*:\s*text\/html|(?:&(?:#x0*3c|#0*60|lt);|%3c)\s*\/?\s*(?:script|iframe|object|embed|svg|img)/i.test(normalized); const compact = multiline ? normalized : normalized.replace(/[\r\n\t]+/g, " "); return { value: compact.trim().slice(0, maxLength), unsafe } }
function migratePersistedState() { const themeValue = localStorage.getItem(STORAGE_KEYS.theme); const resolvedTheme = resolveTheme(themeValue); if (themeValue !== resolvedTheme) localStorage.setItem(STORAGE_KEYS.theme, resolvedTheme); [[STORAGE_KEYS.tasks, state.tasks], [STORAGE_KEYS.history, state.history], [STORAGE_KEYS.trash, state.trash]].forEach(([key, value]) => { const stored = localStorage.getItem(key); if (stored && stored !== JSON.stringify(value)) localStorage.setItem(key, JSON.stringify(value)) }); const storedLock = localStorage.getItem(STORAGE_KEYS.lock); if (storedLock && storedLock !== JSON.stringify(state.lock)) localStorage.setItem(STORAGE_KEYS.lock, JSON.stringify(state.lock)); localStorage.setItem(STORAGE_KEYS.themeMigration, "ocean") }
function openJalaliPicker(input) { if (state.language !== "fa") return; ensureJalaliDatepicker().then(() => { if (document.activeElement === input) window.jalaliDatepicker?.show(input) }) }
function ensureJalaliDatepicker() { if (state.language !== "fa" || window.jalaliDatepicker) { if (window.jalaliDatepicker) startJalaliDatepicker(); return Promise.resolve() } if (state.jalaliAssetsPromise) return state.jalaliAssetsPromise; state.jalaliAssetsPromise = Promise.all([loadLocalStyle("jalaliDatepickerStyle", "./assets/vendor/jalalidatepicker/jalalidatepicker.min.css"), loadLocalScript("jalaliDatepickerScript", "./assets/vendor/jalalidatepicker/jalalidatepicker.min.js")]).then(() => startJalaliDatepicker()).catch(() => { state.jalaliAssetsPromise = null }); return state.jalaliAssetsPromise }
function loadLocalStyle(id, href) { if (document.getElementById(id)) return Promise.resolve(); return new Promise((resolve, reject) => { const link = document.createElement("link"); link.id = id; link.rel = "stylesheet"; link.href = href; link.onload = resolve; link.onerror = reject; document.head.appendChild(link) }) }
function loadLocalScript(id, src) { if (document.getElementById(id)) return Promise.resolve(); return new Promise((resolve, reject) => { const script = document.createElement("script"); script.id = id; script.src = src; script.async = true; script.onload = resolve; script.onerror = reject; document.body.appendChild(script) }) }
